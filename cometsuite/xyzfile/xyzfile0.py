...
class XYZFile0(XYZFileBase):
    ...
    Simulation has been upgraded beyond v0.  `XYZFile0.read_simluation` works,
    but the returned sim will only be partially useful: the parameters will not
    make sense, but the particle array will be OK.
    ...
    _writeRequiredKeywords = ["comet", "kernel", "jd", "nparticles", "datalist"]
    _csFormats = dict(
        radius="d(radius)",
        graindensity="d(graindensity)",
        beta="d(beta)",
        age="d(age)",
        origin="d[2](origin)",
        v_ej="d[3](v_ej)",
    ...
    def _data2rec(self):
        ...
        dtype = []
        dataRe = re.compile(r"(([idc])(\[([0-9]+)\])?\((\w+)\))", re.IGNORECASE)
    ...
    def _readHeader(self):
        ...
        self.header += line
        if ("cometsuite" not in line.lower()) and ("rundynamics" not in line.lower()):
            self.close()
        ...
        paramRe = re.compile(r"^(\w+):\s*(.*)", re.IGNORECASE)
                ...
                if match is not None:
                    ...
                    if parameter == "PROGRAM":
                        if value.lower().find("syndynes") == 0:
                            ...
                            self.close()
                            raise OSError(
                                "I don't understand program " + "parameter:" + value
                            )
                        ...
                        self.params["synbeta"] = np.array(
                            re.split(r"[\s,]+", value), dtype=np.float
                        )
    ...
    def read_simulation(self, n=None):
        ...
        sim.particles = np.fromfile(self._file, dtype=self.rec, count=n).view(
            np.recarray
        )
...
DATA: {}
""".format(
            self.params["units"], self.params["data"]
        )

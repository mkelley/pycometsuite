...
class Ra15(Integrator):
    ...
    def __init__(
        ...
        warnings.warn(Warning("RA15 is untested."))
    ...
    def _parameter_str(self):
        return (
            f"iss={self.iss}, tol={self.tol}, planets={self.planets}, "
            f"planet_lookup={self.planet_lookup}, "
    ...
    def planets(self, p):
        ...
        if len(self._planets) > 0:
            ...
            self._μ_planets = np.array([self.μ_planets[i - 1] for i in self._planets])
        else:
    ...
    def _ra15(self, x, v, beta, dt, et):
        ...
        xc = 1.0 / np.array([2.0, 6, 12, 20, 30, 42, 56, 72])
        vc = 1.0 / np.array([2.0, 3, 4, 5, 6, 7, 8])
            ...
            for m in range(n_iterations):
                ...
                for j in range(1, 8):
                    ...
                    if j == 1:
                        ...
                        g[:, 3] = (
                            ((gk - g[:, 0]) * r[3] - g[:, 1]) * r[4] - g[:, 2]
                        ) * r[5]
                    elif j == 5:
                        g[:, 4] = (
                            (((gk - g[:, 0]) * r[6] - g[:, 1]) * r[7] - g[:, 2]) * r[8]
                            - g[:, 3]
                        ) * r[9]
                    elif j == 6:
                        g[:, 5] = (
                            (
                                (((gk - g[:, 0]) * r[10] - g[:, 1]) * r[11] - g[:, 2])
                                * r[12]
                                - g[:, 3]
                            )
                            * r[13]
            ...
            if first_sequence:
                ...
                if constant_step:
                    ...
                    if t_prime / t_step <= 1.0:
                        ...
                        if restarts > 10:
                            raise ValueError(
                                "Too many time refinements on initial sequence.  Reduce initial step size."
                            )
                        continue
    ...
    def accel(self, r, v, beta, dt, et):
        ...
        if self._perturbations:
            ...
            if self.planet_lookup:
                ...
                ssb_sun = spice.spkssb(10, now, "ECLIPJ2000")[
                    :3
                ]  # wrt solar system barycenter
                for i in range(self._n_planets):
                    # state, ltt = spice.spkez(self._planets[i], now, "ECLIPJ2000", "NONE", 10)
                    r_planet[:, i] = (
                        spice.spkssb(self._planets[i], now, "ECLIPJ2000")[:3] - ssb_sun
                    )

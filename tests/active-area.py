...
params = {
    "box": -1,
    "comet": {"kernel": "None", "name": None, "r": list(target), "v": [0.0, 0.0, 0.0]},
    "cometsuite": "1.0.0",
    "data": ["d(radius)", "d(age)", "d(v_ej)", "d[3](r_f)"],
...
for i in range(params["nparticles"]):
    ...
    v_ej = np.array((np.cos(th) * np.cos(phi), np.cos(th) * np.sin(phi), np.sin(th)))
...
os.system(
    "python3 ../scripts/cs-explore-source iso.xyz -n24 -s300"

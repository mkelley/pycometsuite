...
light = cs.scalers.ScatteredLight(wave, Ap=0.0419)
...
G = np.pi / 0.0419 * Fsim.sum() * rh**2 / S.value * sim.Delta.mean() ** 2
...
F = 0.0419 / np.pi * sigma * S / rh**2 / (1e3 * Delta) ** 2
k = wave / 2 / np.pi
x = sim.radius / k
i = x < 1
F[i] *= (sim.radius / k)[i] ** 4

# Kelley et al. measured 9.06 mag
# This does not include phase correction: 0.938 mag --> 8.12
m = F.sum().to(u.ABmag, u.spectral_density(wave * u.um))
assert np.isclose(m.value, 8.12, atol=0.05)

# calculate with the scaler
m = (Fsim.sum() * u.W / u.m**2 / u.um).to(u.ABmag, u.spectral_density(wave * u.um))
assert np.isclose(m.value, 8.12, atol=0.05)
